...
<body>
    ...
    <script>
        ...
        // Get country name from URL (from Plotly map click)
        let urlParams = new URLSearchParams(window.location.search);
        let selectedCountry = urlParams.get("country") || "France"; // Default to France
        ...
        function fetchCountries() {
            fetch('data_graph.php')
                ...
                .then(data => {
                    ...
                    // Set selected country in the dropdown
                    if (selectedCountry) {
                        countrySelect.value = selectedCountry;
                    }

                    // Fetch and update graphs for the selected country
        ...
        function fetchData(country) {
            fetch(`data_graph.php?pays=${encodeURIComponent(country)}`)
                .then(response => response.json())
                .then(data => {
                    updateCharts(data);
                })
        ...
        function updateCharts(data) {
            ...
            barChart = new Chart(barCtx, {
                ...
                data: {
                    ...
                    datasets: [
                    {
                        ...
                        borderWidth: 1,
                        yAxisID: 'y'  // Uses the left y-axis (for values ~500-1000)
                    },
                    {
                        type: 'bar',
                        label: 'Score de la générosité',
                        data: data.barChart.map(item => item.generosite),
                        backgroundColor: 'rgba(75, 241, 10, 0.51)',
                        borderColor: 'rgb(0, 0, 0)',
                        borderWidth: 1,
                        yAxisID: 'y1' // Uses the right y-axis (for values ~5-10)
                    }
                ]
            },
            options: { 
                ...
                scales: {
                    y: {
                        ...
                        min: 0,
                        ticks: {
                            color: 'blue', // Customize right axis tick color (e.g., red)
                            font: {size: 16} 
                        },
                        // Optionally, set max if needed:
                        // max: 1100,
                    },
                    y1: {
                        type: 'linear',
                        ...
                        min: 0,
                        ticks: {
                            color: 'green', // Customize right axis tick color (e.g., red)
                            font: {size: 16} 
                        },
                        // Optionally, set max if needed:
                        // max: 15,
                        grid: {
                            drawOnChartArea: false // Prevents the grid from this axis overlapping with the left one
                        }
                    }
                }
            }
            ...
            const labels = data.pieChart.flatMap(item => [item.nom_pays + " - Important", 
                                                          item.nom_pays + " - Pas important",
                                                          item.nom_pays + " - Plutot Pas important",
                                                          item.nom_pays + " - Plutot important",
                                                          item.nom_pays + " - Ne Sais Pas",
                                                          item.nom_pays + " - Ne Se Prononce Pas"]);
            const values = data.pieChart.flatMap(item => [item.important, 
                                                          item.pas_important, 
                                                          item.plutot_pas_important,
                                                          item.plutot_important,
                                                          item.ne_sais_pas,
                                                          item.ne_se_prononce_pas]);
                                                          pieChart = new Chart(pieCtx, {
    type: 'polarArea',
    data: {
        labels: labels,
        datasets: [{
            label: "Comparaison Importance Religion",
            data: values,
            backgroundColor: [
                'rgba(255, 99, 132, 0.6)',
                'rgba(54, 162, 235, 0.6)',
    ...
    options: {
        ...
        document.getElementById('countrySelect').addEventListener('change', function () {
            let newCountry = this.value;
            window.location.href = `graph.php?country=${encodeURIComponent(newCountry)}`;
        });
